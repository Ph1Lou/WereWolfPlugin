...
public class RoleManagementLG {
	...
	public void RepartitionrolesLG() {
		...
		while(!joueurs.isEmpty()) { 
			...
			int n =(int) Math.floor(Math.random()*joueurs.size());
	...
	public void recover_rolepower(String playername) {
		...
		if (plg.isRole(RoleLG.LOUP_GAROU_BLANC)) {
			...
			player.sendMessage(main.texte.esthetique("§m", "§e",main.texte.poweruse.get(RoleLG.ENFANT_SAUVAGE)+main.conversion(main.config.value.get(TimerLG.duration))));
		}
		else if (plg.isRole(RoleLG.CUPIDON)) {
			player.sendMessage(main.texte.esthetique("§m", "§e",main.texte.poweruse.get(RoleLG.CUPIDON)+main.conversion(main.config.value.get(TimerLG.duration))));
		}
		else if(plg.isRole(RoleLG.ANGE_DECHU) || plg.isRole(RoleLG.ANGE_GARDIEN) ) {
	...
	public void thief_recover_role(String killername,String playername){
		...
		if (Bukkit.getPlayer(killername)!=null) {
			...
			if (klg.isRole(RoleLG.ENFANT_SAUVAGE)) {
				...
				if(klg.hasPower()) {
					killer.sendMessage(main.texte.esthetique("§m", "§e",main.texte.poweruse.get(RoleLG.ENFANT_SAUVAGE)+main.conversion(main.config.value.get(TimerLG.duration)-main.score.getTimer()+main.config.value.get(TimerLG.role))));
				}
				else {
			...
			if (klg.isRole(RoleLG.CUPIDON)) {
				...
				if(klg.hasPower()) {
					killer.sendMessage(main.texte.esthetique("§m", "§e",main.texte.poweruse.get(RoleLG.CUPIDON)+main.conversion(main.config.value.get(TimerLG.duration)-main.score.getTimer()+main.config.value.get(TimerLG.role))));
				}
				else {
	...
	public void auto_ange() {
		...
		for(String playername:main.playerlg.keySet()) {
			...
			if (plg.isState(State.VIVANT) && (plg.isRole(RoleLG.ANGE_DECHU) || plg.isRole(RoleLG.ANGE_GARDIEN))) {
	...
	public void newLG(String playername) {
		...
		main.boardlg.getTeam("lgteam").addEntry(playername);
	...
	public void lgliste() {
		...
		for(String lgname : main.playerlg.keySet()) {
			...
			if((main.playerlg.get(lgname).isCamp(Camp.LG) || main.playerlg.get(lgname).isRole(RoleLG.LOUP_GAROU_BLANC)) && main.playerlg.get(lgname).isState(State.VIVANT)) {
				
				main.boardlg.getTeam("lgteam").addEntry(lgname);
